...
unsigned UHCIDataHandler_CreateFrameList()
{
	...
	if(KC::MMemManager().AllocatePhysicalPage(&uiFreePageNo) != MEM_SUCCESS)
		return NULL ;
...
{
	unsigned uiFreePageNo ;
	RETURN_X_IF_NOT(KC::MMemManager().AllocatePhysicalPage(&uiFreePageNo), MEM_SUCCESS, UHCIDataHandler_FAILURE) ;
