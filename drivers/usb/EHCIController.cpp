...
bool EHCIDevice::BulkRead(USBulkDisk* pDisk, void* pDataBuf, unsigned uiLen)
{
  ...
  const unsigned uiMaxPacketSize = pDisk->usInMaxPacketSize;
	...
	if(_bFirstBulkRead)
	{
		...
		pBulkInEndPt = pController->CreateDeviceQueueHead(uiMaxPacketSize, pDisk->uiEndPointIn, devAddr) ;
	}
	else
	{
		int i ;
		...
		EHCIDataHandler_CleanAysncQueueHead(pBulkInEndPt) ;
	}
	...
	EHCITransaction aTransaction(pBulkInEndPt, _ppBulkReadTDs[ 0 ]);

	if(!aTransaction.PollWait())
	{
		printf("\n Bulk Read Transaction Failed: ") ;
		EHCIController_DisplayTransactionState(pBulkInEndPt, _ppBulkReadTDs[0]) ;
		pController->DisplayStats() ;
		return false ;
	}
...
bool EHCIDevice::BulkWrite(USBulkDisk* pDisk, void* pDataBuf, unsigned uiLen)
{
  ...
  const unsigned uiMaxPacketSize = pDisk->usOutMaxPacketSize;
	...
	if(_bFirstBulkWrite)
	{
		...
		pBulkOutEndPt = pController->CreateDeviceQueueHead(uiMaxPacketSize, pDisk->uiEndPointOut, devAddr) ;
	}
	else
	{
		int i ;
		...
		EHCIDataHandler_CleanAysncQueueHead(pBulkOutEndPt) ;
	}
	...
	EHCITransaction aTransaction(pBulkOutEndPt, _ppBulkWriteTDs[ 0 ]);

	if(!aTransaction.PollWait())
	{
		printf("\n Bulk Write Transaction Failed: ") ;
		EHCIController_DisplayTransactionState(pBulkOutEndPt, _ppBulkWriteTDs[0]) ;
		pController->DisplayStats() ;
		return false ;
	}
...
EHCIDevice::EHCIDevice(EHCIController* controller) 
  : pController(controller),
    pControlQH(controller->CreateDeviceQueueHead(64, 0, 0)),
